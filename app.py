...
[data-testid="stMetric"]:hover {
    ...
    box-shadow: 0px 10px 20px rgba(106, 13, 173, 0.15) !important;
}
...
.logo-icon { width: 65px; height: 65px; border-radius: 18px; display: flex; align-items: center; justify-content: center; font-size: 34px; background: linear-gradient(135deg, rgba(255,255,255,0.18), rgba(255,255,255,0.05)); border: 1px solid rgba(255,255,255,0.2); box-shadow: 0 8px 25px rgba(0,0,0,0.25), inset 0 0 20px rgba(255,255,255,0.05); }
...
.stTabs [data-baseweb="tab"] p { color: rgba(255,255,255,0.6) !important; font-size: 14px !important; font-weight: 600 !important; letter-spacing: 0.5px; }
...
# ------------------------------------------
# TAB 3: Prediction Result (Neat Dashboard Style)
# ------------------------------------------
with tab_pred:
    ...
    st.markdown("""
    <style>
    /* Clean white dashboard cards similar to the target image */
    .dash-card {
        background: #ffffff;
        border-radius: 12px;
        padding: 18px 22px;
    ...
    .dash-card-title {
        color: #8a7a99; font-size: 13px; font-weight: 700; text-transform: uppercase; margin-bottom: 8px;
    }
    .dash-card-val {
        color: #1a052b; font-size: 26px; font-weight: 800; line-height: 1.2;
    }
    .dash-card-icon {
        font-size: 24px; padding: 10px; border-radius: 10px;
        background: #f7f2fb; color: #6A0DAD; display: inline-flex; align-items: center; justify-content: center;
        margin-bottom: 12px; width: 45px; height: 45px;
    }

    /* Main Prediction Card Style */
    .main-pred-card {
        background: linear-gradient(135deg, #ffffff 0%, #fdfbff 100%);
        border-radius: 16px; padding: 25px 30px;
        box-shadow: 0 8px 25px rgba(106,13,173,0.08);
        border-left: 6px solid #6A0DAD;
        margin-bottom: 20px;
    }
    .main-pred-title { color: #8a7a99; font-size: 14px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 8px;}
    .main-pred-value { color: #3a0a63; font-size: 48px; font-weight: 900; line-height: 1.1; margin-bottom: 12px;}
    ...
    .strategy-card { background: #fffbfa; border-radius: 12px; padding: 18px 22px; border: 1px solid #ffe8e3; border-left: 4px solid #ff6b6b; }
    ...
    # Page 2: Result (Clean Dashboard Style)
    else:
        ...
        col_back, col_space, col_export = st.columns([1, 4, 1])
        with col_back:
            if st.button("⬅ Back", use_container_width=True):
                st.session_state.show_prediction = False
                st.rerun()
        with col_export:
            st.download_button("📥 Export CSV", data=csv_data, file_name=f"prediction_{selected_model_name.replace(' ', '_').lower()}.csv", mime="text/csv", use_container_width=True)
        ...
        with main_left:
            # Huge Prediction Result Card
            st.markdown(f"""
            <div class="main-pred-card">
                <div class="main-pred-title">Predicted Player Engagement</div>
                <div class="main-pred-value">{prediction}</div>
                <div class="pred-model-badge">⚡ Powered by {selected_model_name}</div>
            </div>
            """, unsafe_allow_html=True)

            # Profile Summary Cards (KPI style matching target image)
            st.markdown('<div class="section-header"><span class="dot"></span><span class="label"> Player Profile Overview</span></div>', unsafe_allow_html=True)
            
            c1, c2, c3 = st.columns(3)
            with c1:
                st.markdown(f'<div class="dash-card"><div class="dash-card-icon">👤</div><div class="dash-card-title">Demographics</div><div class="dash-card-val" style="font-size:16px;">{profile["Age"]} yrs • {profile["Gender"]} <br><span style="font-weight:400; color:#666;">{profile["Location"]}</span></div></div>', unsafe_allow_html=True)
            with c2:
                st.markdown(f'<div class="dash-card"><div class="dash-card-icon">🎮</div><div class="dash-card-title">Game Setup</div><div class="dash-card-val" style="font-size:16px;">{profile["Game Genre"]} <br><span style="font-weight:400; color:#666;">Diff: {profile["Difficulty"]} • Pay: {profile["Purchases"]}</span></div></div>', unsafe_allow_html=True)
            with c3:
                st.markdown(f'<div class="dash-card"><div class="dash-card-icon">⏱️</div><div class="dash-card-title">Play Behavior</div><div class="dash-card-val" style="font-size:16px;">{profile["Play Time"]} <br><span style="font-weight:400; color:#666;">{profile["Sessions"]} • {profile["Avg Session"]}</span></div></div>', unsafe_allow_html=True)

        with main_right:
            # Probability Chart
            with st.container(border=True):
                st.markdown('<div class="section-header"><span class="dot"></span><span class="label"> Confidence Score</span></div>', unsafe_allow_html=True)
...
# ------------------------------------------
# TAB 4: Model Selection & Rationale
# ------------------------------------------
with tab_why:
    ...
    with col_table:
        with st.container(border=True):
            ...
            st.markdown("""
            **Key Findings:**
            *   **XGBoost (Winner):** Flawlessly mapped non-linear player behavioral boundaries with **96.9% Accuracy**.
            *   **Random Forest:** A strong runner-up (95.1%). Proved ensemble bagging is highly effective for this dataset.
            *   **Logistic Regression:** Solid baseline (90.4%), proving clear linear signals exist.
            *   **KNN:** Struggled the most (84.6%) due to the high-dimensional feature space.
            """)
